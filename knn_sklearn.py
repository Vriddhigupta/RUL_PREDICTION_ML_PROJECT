...
#k value that has smallest rmse value
min_rmse_val=rmse_val.index(min(rmse_val))

#sklearn model
model = neighbors.KNeighborsRegressor(n_neighbors = min_rmse_val)
model.fit(X_train, y_train)  #fit the model
prediction=model.predict(X_test) #make prediction on test set
